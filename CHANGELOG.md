...
### Added
- Finish Line (⌥⌘F, toolbar 🏁 button, Attention menu) — a live production burn-down dashboard computed from the board in real time (never stale). Tiers the work the way the FINISH-LINE plan does: **Next 3** (the recommendation — today's P1 breakfix and ready P1 stories first; epics excluded as themes), then **Blockers (P1)**, **In flight** (with a one-click *Park* to drop your started-count), **Polish (P2)**, **Later (P3)** — each collapsible with advice subtitles. Product / Cockpit / All scope toggle, search, "Brian only" filter, a header pulse (blockers / in-flight / Brian counts), per-row age ("today" / "3d"), and an **Inflow-by-day** panel so the rate is visible, not just the pile. Fully interactive (click → edit, ▶ → start a Claude pane, right-click → status, *Mark done — no longer required*) and an **Export .md** button that writes a shareable FINISH-LINE.md snapshot.
